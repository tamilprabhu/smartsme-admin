...
function ProductList() {
  ...
  const fetchProducts = async () => {   
    const endpoint = `/product?page=${page}&itemsPerPage=10&search=${search}`
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product.prodSequence}>
              ...
              <td>
                <Link
                  to={`/product/${product.prodSequence}`}
                  className="btn btn-sm btn-info me-2"
                >
                  View
                </Link>
                <Link
                  to={`/product/${product.prodSequence}/edit`}
                ...
                <Button
                  size="sm"
                  variant="danger"
                  onClick={() => handleDelete(product.prodSequence)}
                >
                  Delete
                </Button>
              </td>
